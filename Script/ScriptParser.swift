...
class ScriptParser {
  ...
  var rest_recognizers: [(String, (String, ScriptCallback) -> ScriptCommand)] = [
    ...
    (
      "require",
      { rest, callback in
        return LoadFilterCommand(
          callback: callback, pattern: rest,
          loadFilterType: LoadFilterCommand.LoadFilterType.Required)
      }
    ),
    (
      "exclude",
      { rest, callback in
        return LoadFilterCommand(
          callback: callback, pattern: rest,
          loadFilterType: LoadFilterCommand.LoadFilterType.Excluded)
      }
    ),
    (
      "clearFilters",
      { rest, callback in
        return LoadFilterCommand(
          callback: callback, pattern: rest, loadFilterType: LoadFilterCommand.LoadFilterType.Clear)
      }
    ),
    (
      "requireToday",
      { rest, callback in
        return LoadFilterCommand(
          callback: callback, pattern: rest,
          loadFilterType: LoadFilterCommand.LoadFilterType.RequireToday)
      }
    ),
    ...
    (
      "sort",
      { rest, callback in return SortByFieldsCommand(callback: callback, fieldsString: rest) }
    ),
  ]
  ...
  var token_recognizers: [(String, Int, ([String], ScriptCallback) -> ScriptCommand)] = [
    ...
    (
      "truncate", 1,
      { tokens, callback in
        return TruncateCommand(callback: callback, maxLength: Int(tokens[1]) ?? 256)
      }
    ),
    (
      "limit", 1,
      { tokens, callback in
        return LimitCommand(callback: callback, limit: Int(tokens[1]) ?? 1_000_000)
      }
    ),
    (
      "d", 1,
      { tokens, callback in
        return DetectDuplicatesCommand(callback: callback, threshold: Int(tokens[1]) ?? 20)
      }
    ),
    (
      "dateFormat", 1,
      { tokens, callback in return DateFormatCommand(callback: callback, text: tokens[1]) }
    ),
    (
      "@", 2,
      { tokens, callback in
        return AddFieldCommand(callback: callback, fieldToAdd: tokens[1], fieldToMatch: tokens[2])
      }
    ),
    (
      "/f", 1,
      { tokens, callback in return DivideByFieldCommand(callback: callback, field: tokens[1]) }
    ),
    (
      "replace", 2,
      { tokens, callback in
        return ReplaceCommand(callback: callback, oldText: tokens[1], newText: tokens[2])
      }
    ),
    (
      "sleep", 1,
      { tokens, callback in return SleepCommand(callback: callback, seconds: Int(tokens[1]) ?? 10) }
    ),
